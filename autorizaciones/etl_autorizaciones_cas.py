...
def get_sql_semana_actual(lunes_str):
    return f"""
    SELECT *
    FROM analitica.autorizacionesCasView
    WHERE fechaSolicitud >= '{lunes_str}'
      AND fechaSolicitud < NOW();
    """

project_id_product = 'ia-bigquery-397516'
dataset_id_autorizaciones = 'autorizaciones'
table_name_autorizaciones = 'autorizacionesCas'
...
def convert_date(df):
    for col in ['fechaSolicitud', 'fechaImpresion', 'fechaVencimientoAutorizacion']:
...
def convert_number(df):
    int_cols = [
        'cantidadPrestacion', 'idCodigoTipoCobro', 'idCodigoOrigenServicio',
        'idTipoOrden', 'idCodigoTipoContrato', 'idEstadoOrden',
        'idCodigoLugarRecaudo', 'edadCapita'
    ]
    for col in int_cols:
        if col in df.columns:
...
print("=" * 60)
print(" ETL SEMANAL — autorizacionesCas")
print("=" * 60)
...
WHERE fechaSolicitud >= TIMESTAMP('{lunes_str}')
"""
...
t0 = time.time()
df_cas_bd = func_process.load_df_server(get_sql_semana_actual(lunes_str), 'analitica')
log(f"Extracción completada en {time.time() - t0:.2f}s — {len(df_cas_bd):,} filas", "✅")

log("Normalizando columnas a camelCase...", "🔤")
df_cas_bd = columns_to_camel_case(df_cas_bd)

log("Convirtiendo tipos (fechas y numéricos)...", "🛠️")
df_cas_bd = convert_date(df_cas_bd)
df_cas_bd = convert_number(df_cas_bd)
...
print(f"{len(df_cas_bd)},{TABLA_BIGQUERY},{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")